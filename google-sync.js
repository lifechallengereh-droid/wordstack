/* WordStack Google Sheets sync v7.2
   Target spreadsheet: 플래시카드_단어장_양식_v6 / Flashcards
   OAuth access tokens are kept in memory only. */
const WORDSTACK_GOOGLE_SHEET_ID='1Xlc6nummFrvCLkDiOMUtj_u4SpHwwwDlFcf95-N4g5I';
const WORDSTACK_GOOGLE_SHEET_NAME='Flashcards';
const WORDSTACK_GOOGLE_SHEET_URL='https://docs.google.com/spreadsheets/d/1Xlc6nummFrvCLkDiOMUtj_u4SpHwwwDlFcf95-N4g5I/edit';
const WORDSTACK_GOOGLE_SCOPE='https://www.googleapis.com/auth/spreadsheets';
const GOOGLE_HEADERS=['EnglishWord','IPA','EnglishExample','KoreanMeaning','KoreanExample','Category','Chapter','Tags'];
...
function googleRowToCard(row){
  const vals=[...row]; while(vals.length<8) vals.push('');
  return {
    englishWord:String(vals[0]||'').trim(), ipa:String(vals[1]||'').trim(), englishExample:String(vals[2]||'').trim(),
    koreanMeaning:String(vals[3]||'').trim(), koreanExample:String(vals[4]||'').trim(), category:String(vals[5]||'기본 단어장').trim()||'기본 단어장',
    chapter:String(vals[6]||'').trim(), tags:String(vals[7]||'').trim()
  };
}
function googleCardToRow(c){return [c.englishWord||'',c.ipa||'',c.englishExample||'',c.koreanMeaning||'',c.koreanExample||'',c.category||'',c.chapter||'',c.tags||'']}
function googleContentChanged(card,rowCard){return ['englishWord','ipa','englishExample','koreanMeaning','koreanExample','category','chapter','tags'].some(k=>String(card[k]||'')!==String(rowCard[k]||''))}
...
function sheetValuesUrl(range){return `https://sheets.googleapis.com/v4/spreadsheets/${encodeURIComponent(WORDSTACK_GOOGLE_SHEET_ID)}/values/${encodeURIComponent(range)}`}
async function fetchGoogleRows(){
  const j=await gfetch(sheetValuesUrl(`${WORDSTACK_GOOGLE_SHEET_NAME}!A1:H10000`));
  const values=j?.values||[];
  if(values.length&&GOOGLE_HEADERS.some((h,i)=>String(values[0]?.[i]||'').trim()!==h))throw new Error('Google 시트의 첫 행 머리글이 WordStack 양식과 다릅니다. A1:H1을 확인해주세요.');
  return values.slice(1).filter(r=>String(r?.[0]||'').trim()&&String(r?.[3]||'').trim());
}
async function ensureSheetRows(requiredRows){
  const meta=await gfetch(`https://sheets.googleapis.com/v4/spreadsheets/${encodeURIComponent(WORDSTACK_GOOGLE_SHEET_ID)}?fields=sheets(properties(sheetId,title,gridProperties(rowCount)))`);
  ...
  if(requiredRows>current){
    await gfetch(`https://sheets.googleapis.com/v4/spreadsheets/${encodeURIComponent(WORDSTACK_GOOGLE_SHEET_ID)}:batchUpdate`,{
...
async function writeGoogleRows(rows){
  ...
  await gfetch(sheetValuesUrl(`${WORDSTACK_GOOGLE_SHEET_NAME}!A2:H${rowCount}`)+':clear',{method:'POST',headers:{'Content-Type':'application/json'},body:'{}'});
  await gfetch(sheetValuesUrl(`${WORDSTACK_GOOGLE_SHEET_NAME}!A1:H${totalRows}`)+'?valueInputOption=RAW',{
    method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify({range:`${WORDSTACK_GOOGLE_SHEET_NAME}!A1:H${totalRows}`,majorDimension:'ROWS',values:[GOOGLE_HEADERS,...rows]})
  });
}
...
async function googleSync({interactive=true,silent=false}={}){
  ...
  try{
    ...
    state.googleSync={lastSyncAt:new Date().toISOString(),spreadsheetId:WORDSTACK_GOOGLE_SHEET_ID,sheetName:WORDSTACK_GOOGLE_SHEET_NAME};
...
window.addEventListener('DOMContentLoaded',()=>{
  ...
  const open=$('openGoogleSheetBtn');if(open)open.onclick=()=>window.open(WORDSTACK_GOOGLE_SHEET_URL,'_blank','noopener');
